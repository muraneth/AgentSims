...
class CommandBase(Base):
    ...
    def _execute(self, params=None):
        ...
        # token = None
        # if self.is_check_token() and 'token' in params:
        #     token = params['token']
        #     try:
    ...
    #     session_model = self.get_single_model('Session', create=False)
    #     if session_model is None or token != session_model.get_token():
    ...
    # def gen_token(self, id):
    #     return f"{id}:{self.get_nowtime()}:{randbytes(20).hex()}"
