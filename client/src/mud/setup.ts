...
import { SystemTypes } from "@llmtown/contracts/types/SystemTypes";
import { config } from "./config";
import { contractComponents, clientComponents } from "./components";
import { world } from "./world";
import { SystemAbis } from "@llmtown/contracts/types/SystemAbis.mjs";
...
export const setup = async () => {
  ...
  if (!config.devMode && config.faucetServiceUrl) {
    ...
    components.Position.update$.subscribe((update:any)=>{
      console.log("position compoents update:", update)
    });
  }
